...
Agent = NamedTuple('Agent', [('initial_position', Tuple[int, int]),
                             ...
                             ('moving', bool),
                             ('initial_shortest_path', List[Waypoint]),
...
def load_env_agent(agent_tuple: Agent):
     return EnvAgent(
                        ...
                        initial_shortest_path = agent_tuple.initial_shortest_path,
...
class EnvAgent:
    ...
    initial_shortest_path = attrib(default=None, type=List[Waypoint])
    ...
    def to_agent(self) -> Agent:
        return Agent(initial_position=self.initial_position, 
                     ...
                     target=self.target,
                     moving=self.moving,
                     initial_shortest_path = self.initial_shortest_path,
    ...
    def get_travel_time_on_shortest_path(self, distance_map) -> int:
        ...
        if shortest_path is not None:
            ...
            if self.initial_shortest_path is None:
                self.initial_shortest_path = distance
        else:
            distance = 0
    ...
    def get_initial_shortest_path(self, distance_map):
        from flatland.envs.rail_env_shortest_paths import get_initial_shortest_path
        return get_initial_shortest_path(distance_map=distance_map, agent_handle=self.handle)[self.handle]
