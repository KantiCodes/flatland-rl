...
class RailEnv(Environment):
    ...
    def _handle_end_reward(self, agent: EnvAgent, should_print=False, reward_shaping=False) -> int:
        '''
        ...
        # # Agents not done (arrival_time is None)
            ...
            if (agent.state.is_off_map_state()):
                ...
                # print(f'NOT DEPARTURED: {reward}')

                # print(f'This is the travel time at the end of the episode {reward}')
            #     reward = -5
            # # Departed but never reached
            if (agent.state.is_on_map_state()) and reward_shaping==False:
                # reward = (agent.initial_shortest_time/agent.get_travel_time_on_shortest_path())/500
                reward = -agent.get_travel_time_on_shortest_path(self.distance_map)
                # print(f"reward is on map but not departured {reward}")

                print(f'DEPARTURED: {reward}')

        if should_print:
            print(f'this is the agent: {agent.handle}')
            print(f"this is the target {agent.target}")
            print(f"this is the position {agent.position}")
    ...
    def update_step_rewards(self, i_agent:int):
        """
        Update the rewards dict for agent id i_agent for every timestep
        """
        print("step 2")
        agent: EnvAgent = self.agents[i_agent]
        # if agent.initial_shortest_path is None:
        #     # print(f"calculating the shortest path for agent: {agent.handle}")
        #     agent.initial_shortest_path = agent.get_initial_shortest_path(self.distance_map)
        #     # print(f"the initial path is{len(agent.initial_shortest_path)} \n")
        # else:
            # reward = (agent.get_travel_time_on_shortest_path(self.distance_map) / len(agent.initial_shortest_path)) * -0.25
        reward = -agent.get_travel_time_on_shortest_path(self.distance_map)
        # print(f"this is the update step reward: {reward}")
        self.rewards_dict[agent.handle] = reward


    def end_of_episode_update(self, have_all_agents_ended, reward_shaping=False):
        """ 
        ...
        if have_all_agents_ended or \
           ( (self._max_episode_steps is not None) and (self._elapsed_steps >= self._max_episode_steps)):
            ...
            for i_agent, agent in enumerate(self.agents):
                ...
                reward = self._handle_end_reward(agent, reward_shaping=reward_shaping)
                # print(f'the reward {reward}')
                # If we are using reward shaping we want to set the reward to 0 when trains arrive at its destination
                if reward_shaping:
                    if reward == 0:
                        self.rewards_dict[i_agent] = 0
                    elif reward is None:
                        reward = 0  # We already updated the current reward during update_step
                print(f'the reward: {reward}')
            ...
            # if have_all_agents_ended and reward_shaping:
            #     # print(f"before update: {self.rewards_dict}")
            #     for k in self.rewards_dict:
            #         self.rewards_dict[k] *= 2
            #     # print(f"after update: {self.rewards_dict}")
    ...
    def step(self, action_dict_: Dict[int, RailEnvActions], reward_shaping=False):
        """
        ...
        #TODO we are trying to increase the gain of reward when all trains arrive
        # if reward != 0 then all_agents_done = False

        # if all_agents_done:
        #     reward *= env.number_of_action
        ...
        for agent in self.agents:
            ...
            # if reward_shaping:
            #     self.cancellation_time_buffer = 1
        ...
        self.end_of_episode_update(have_all_agents_ended, reward_shaping=reward_shaping)

        self._update_agent_positions_map()

        # print(f'ths is the reward: {self.rewards_dict}')
